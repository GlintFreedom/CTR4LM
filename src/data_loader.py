...
def load_data(args):
    ...
    print_info()
    user_prompts, item_prompts = load_prompts(args)
    return train_data, eval_data, test_data, neighbor, n_node, n_relation, user_prompts, item_prompts
...
def load_prompts(args):
    ...
    user_prompt_file = '../data/' + args.dataset + '/user_prompts.txt'
    item_prompt_file = '../data/' + args.dataset + '/item_prompts.txt'
    user_prompts = dict()
    item_prompts = dict()
    with open(user_prompt_file, 'r') as f:
        for line in f:
            idx, prompt = line.strip().split('\t')
            user_prompts[int(idx) + n_entity] = prompt
    with open(item_prompt_file, 'r') as f:
        for line in f:
            idx, prompt = line.strip().split('\t')
            item_prompts[int(idx)] = prompt
