...
def train(args, data_info):
    ...
    for step in range(args.n_epoch):
        ...
        while start < train_data.shape[0]:
            ...
            scores, crossloss = model(*_get_feed_data(args, train_data, neighbor_dict, start, start + args.batch_size))
            loss = loss_func(scores, labels) + 0.001 * crossloss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            start += args.batch_size
        train_auc, train_f1 = ctr_eval(args, model, train_data, neighbor_dict)
        eval_auc, eval_f1 = ctr_eval(args, model, eval_data, neighbor_dict)
        test_auc, test_f1 = ctr_eval(args, model, test_data, neighbor_dict)
        # wandb记录数据集表现
...
def ctr_eval(args, model, data, neighbor_dict):
    auc_list = []
    f1_list = []
    model.eval()
    start = 0
    while start < data.shape[0]:
        ...
        scores, _ = model(*_get_feed_data(args, data, neighbor_dict, start, start + args.batch_size))
...
def _init_model(args, data_info):
    ...
    model = MODEL(args, n_node, n_relation)
    if args.use_cuda:
        model.cuda()
